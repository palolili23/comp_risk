total_ipwcs_pr <- function(data,
  ...
  if(is.null(factors_num_cens)){
    data_long %<>% 
      mutate(cens_num = 1) } else {
    model_num_cens <-
      reformulate(termlabels = factors_num_cens, response = "no_cens")
    num_cens <-
      glm(model_num_cens,
          data = subset(data_long, time > 50),
          family = quasibinomial())
    data_long %<>%
      mutate(cens_num = predict(num_cens, data_long, type = "response"))
  }
  ...
  data_long %<>%
    ...
    group_by(id) %>%
    mutate(cens_num_cum = cumprod(cens_num),
